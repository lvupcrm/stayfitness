    ...
    const fetchPageData = async () => {
      try {
        ...
        if (!hasSupabase) {
          ...
          console.log('🔧 Using mock CMS data for Reviews page')
          const reviewsPageData = mockPagesList.find(p => p.slug === 'reviews')
          setPageData(reviewsPageData || null)
          setIsLoading(false)
          return
        }

        const response = await fetch('/api/cms/pages/reviews')
        if (!response.ok) {
        ...
        console.log('🔧 Falling back to mock CMS data for Reviews page')
        const reviewsPageData = mockPagesList.find(p => p.slug === 'reviews')
        setPageData(reviewsPageData || null)
      } finally {
        setIsLoading(false)
