    ...
    const fetchPageData = async () => {
      try {
        ...
        if (!hasSupabase) {
          ...
          console.log('🔧 Using mock CMS data for Locations page')
          const locationsPageData = mockPagesList.find(p => p.slug === 'locations')
          setPageData(locationsPageData || null)
          setIsLoading(false)
          return
        }

        const response = await fetch('/api/cms/pages/locations')
        if (!response.ok) {
        ...
        console.log('🔧 Falling back to mock CMS data for Locations page')
        const locationsPageData = mockPagesList.find(p => p.slug === 'locations')
        setPageData(locationsPageData || null)
      } finally {
        setIsLoading(false)
